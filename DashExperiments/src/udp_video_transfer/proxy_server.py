...
def make_request_handler(proxy_server_port, video_server_port, dir_name):
    class ProxyServer(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            logger.debug("Client address: {}".format(self.client_address))            
            logger.debug('[ Stream name {} ] requesting {}'.format(self.stream_name, self.path))
        ...
        def get_udp_chunk(self, file_name):
            ...
            logger.info("Getting UDP chunk {}".format(request))
...
def init(args):
    ...
    logger.info("Making temporary dirs")
    ...
    proxyServer = HTTPServer(('', args.proxy_server_http_port), handler_class)
...
def main():
    ...
    parser.add_argument('proxy_no', type=str)
    parser.add_argument('--display', action="store_true")
    parser.add_argument('--debug', action="store_true")
    args=parser.parse_args()

    form = '%(asctime)s - %(name)s - %(levelname)s - %(message)s'
    logfile = os.path.join(args.folder_log, "log/proxy_{}.log".format(args.proxy_no))
    ...
    logger = logging.getLogger("PROXY {}".format(args.proxy_no))
