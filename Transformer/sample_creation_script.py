...
if __name__ == "__main__":
    for provider in os.listdir('../Data/FeedbackResults'):
        ...
        if 'Phone' in provider: ### Phone data is parsed a bit different as we can't access the html5 element
            ...
            buffer_max_seconds = np.around((np.percentile(buffer_level_estimate, 90).astype(int) / 10.)) * 10 ### Reference buffer which we can fill. We have to account for noise
            for evaluation_frame in tqdm(os.listdir(parent_path),
                                         'Conversion buffer size : %s' % provider):
                local_path = os.path.join(parent_path, evaluation_frame)
                try:
                    video_information_csv_path = find_video_info(local_path.replace('62085745', '62092214') + '/') ### Naming issue with the vimeo data
            ...
            buffer_max_seconds = max(buffer_max_seconds, 60) ### Reference buffer which we can fill. We have to account for noise
        ...
        for frame in streaming_evaluation_dataframe_list:
            cond = len(trajectory_list.trajectory_sample_association[frame.name]) == len(frame.streaming_session_evaluation)
            assert cond
