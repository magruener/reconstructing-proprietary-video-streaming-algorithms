...
"""
Taken from the pensieve repo
"""

class SingleActorNetwork(object):
    """
...
class SinlgeCriticNetwork(object):
    """
...
def compute_gradients(s_batch, a_batch, r_batch, terminal, actor, critic):
    """
    batch of s, a, report_var is from samples in a sequence
...
def discount(x, gamma):
    """
    Given vector input_tuple, computes a vector y such that
    y[i] = input_tuple[i] + gamma * input_tuple[i+1] + gamma^2 input_tuple[i+2] + ...
    """
    out = np.zeros(len(x))
    out[-1] = x[-1]
    ...
    # scipy.signal.lfilter([1],[1,-gamma],input_tuple[::-1], axis=0)[::-1]
    return out


def compute_entropy(x):
    """
    Given vector input_tuple, computes the entropy
    H(input_tuple) = - sum( p * log(p))
    """
    H = 0.0
