...
VIDEO_KBIT_RATE = [200, 1100, 2000, 2900, 3800, 4700, 5600, 6500, 7400, 8400]
...
TRAIN_TRACES = './traces_train/'
TRAIN_VIDEOS = './videos_train/'
NN_MODEL = './models/nn_model_ep_96000.ckpt'
...
def testing(epoch, nn_model, log_file):
    ...
    os.system('rm -report_var ' + TEST_LOG_FOLDER)
...
def central_agent(net_params_queues, exp_queues):
    ...
    with tf.Session() as sess, open(LOG_FILE + '_test', 'w') as test_log_file:
        ...
        summary_ops, summary_vars = multi_a3c.build_summaries()
        ...
        if nn_model is not None:  # nn_model is the path to file
            ...
            epoch = int(nn_model.split('_')[-1].split('.')[0])
            print("Model restored. Epoch : %d" % epoch)
        else:
            epoch = 0
...
def agent(agent_id, net_params_queue, exp_queue):
    net_env = Environment(random_seed=agent_id,
                          fixed_env=False,
    ...
    with tf.Session() as sess, open(LOG_FILE + '_agent_' + str(agent_id), 'w') as log_file:
        ...
        while True:  # experience video streaming forever
            ...
            reward = VIDEO_KBIT_RATE[action] / M_IN_K \
                     - REBUF_PENALTY * rebuf \
                     - SMOOTH_PENALTY * np.abs(VIDEO_KBIT_RATE[action] -
                                               VIDEO_KBIT_RATE[last_action]) / M_IN_K
            ...
            state[0, -1] = VIDEO_KBIT_RATE[action] / float(np.max(VIDEO_KBIT_RATE))  # last quality
            ...
            entropy_record.append(multi_a3c.compute_entropy(action_prob[0]))

            # log time_stamp, bit_rate, buffer_size, reward
            log_file.write(str(time_stamp) + '\t' +
                           str(VIDEO_KBIT_RATE[action]) + '\t' +
                           str(buffer_size) + '\t' +
                           str(rebuf) + '\t' +
...
def main():
    ...
    assert len(VIDEO_KBIT_RATE) == A_DIM
